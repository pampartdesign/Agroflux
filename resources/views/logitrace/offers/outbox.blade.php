...
<head>
  ...
  <title>Offers Outbox - AgroFlux</title>
...
<div class="max-w-6xl mx-auto py-10 px-4">
  <div class="flex items-start justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold">Offers Outbox</h1>
      <p class="text-sm text-gray-600">Offers your trucker organization has sent.</p>
    </div>
  ...
  <div class="bg-white shadow rounded-xl overflow-hidden">
    <table class="w-full text-sm">
      <thead class="bg-gray-100">
        <tr>
          <th class="text-left p-3">Farmer Org</th>
      ...
      @forelse($offers as $o)
        <tr class="border-t">
          <td class="p-3 font-medium">{{ $o->farmerTenant?->name ?? ('Tenant #'.$o->farmer_tenant_id) }}</td>
      ...
      @empty
        <tr class="border-t"><td colspan="5" class="p-6 text-sm text-gray-600">No offers sent.</td></tr>
      @endforelse
      </tbody>
    </table>
  </div>
